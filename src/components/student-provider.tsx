...
import { buildTicketLink, formatTicketNumber, generateQrToken } from "@/lib/students";
import { safeRandomId } from "@/lib/utils";
import { SharedStudentPayload, TicketStatus, TicketStudent } from "@/lib/types";
...
type StudentStore = {
  ...
  importStudent: (payload: SharedStudentPayload) => TicketStudent | null;
  shareLink: (student: TicketStudent) => string;
};
...
export function StudentProvider({ children }: { children: React.ReactNode }) {
  ...
  const value: StudentStore = {
    ...
    deleteStudent,
    markEntered,
    importStudent: (payload: SharedStudentPayload) => {
      const normalizedToken = payload.qrToken.toLowerCase();
      const existing = students.find((s) => s.qrToken.toLowerCase() === normalizedToken);
      ...
      const newStudent: TicketStudent = {
        ...
        status: payload.status ?? TicketStatus.NOT_ENTERED,
    ...
    shareLink: (student: TicketStudent) => buildTicketLink(student)
  };
